    ...
    formFields.forEach((field, index) => {
        ...
        if (field.type === "text") {
            ...
            (_a = field.options) === null || _a === void 0 ? void 0 : _a.forEach((option) => {
                ...
                optionInput.type = field.type; // Proper type casting
...
function saveForm() {
    ...
    if (savedForms.some((form) => form.name === formName)) {
...
function renderSavedForms() {
    ...
    const savedForms = JSON.parse(localStorage.getItem("forms") || "[]");
    savedForms.forEach((form, formIndex) => {
...
function editFieldInSavedForm(formIndex, fieldIndex) {
    const savedForms = JSON.parse(localStorage.getItem("forms") || "[]");
...
function deleteFieldInSavedForm(formIndex, fieldIndex) {
    const savedForms = JSON.parse(localStorage.getItem("forms") || "[]");
